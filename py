...
class Downloadproxies():
    ...
    def get_special2(self):
        for i in range(json.loads(requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=100000000000000').text)["summary"]['proxiesOnline'] // 100):
            proxies = json.loads(requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=https&timeout=100000000000000'.format(i)).text)
